...
class SubwayApp(SubwayMapMixin, tk.Tk):
    def __init__(self):
        ...
        style.map('Transfer.Treeview', background=[('selected', '#ADD8E6')],
                  foreground=[('selected', 'black')])  # Light blue background on select
    ...
    def create_line_station_tab(self):
        ...
        self.transfer_button = ttk.Button(transfer_info_frame, text="查找可换乘线路", state=tk.DISABLED,
                                          command=self.show_transfer_lines)
    ...
    def show_transfer_lines(self):
        ...
        if station_info and station_info['is_transfer']:
            ...
            if transfer_lines:
                ...
                messagebox.showinfo("无其他换乘线路",
                                    f"在数据库中未找到站点 '{station_name}' 的其他换乘线路信息。\n(可能原因：数据库中其他线路未将此站标记为换乘站，或无同名站点)")
                if hasattr(self, 'transfer_label'):
        ...
        if station_id:
            ...
            if schedules:
                for schedule in schedules:
                    ...
                    arrival_str = arrival_val.strftime('%H:%M:%S') if hasattr(arrival_val, 'strftime') else str(
                        arrival_val) if arrival_val else "---"
                    departure_str = departure_val.strftime('%H:%M:%S') if hasattr(departure_val, 'strftime') else str(
                        departure_val) if departure_val else "---"
                ...
                self.schedule_display_tree.insert('', tk.END, values=(f'当天 ({selected_date_str})', '无时刻信息', '', ''),
                                                  tags=('placeholder',))
    ...
    def create_train_admin_tab(self):
        ...
        self.train_admin_tree = ttk.Treeview(list_frame, columns=cols_train, show='headings', height=10,
                                             selectmode="browse")
    ...
    def update_train_status(self):
        ...
        confirm = messagebox.askyesno("Confirm Update",
                                      f"确定要将列车 ID {train_id} 的状态从 '{current_status}' 更新为 '{new_status}' 吗？\n(此操作将触发日志记录)")
    ...
    def create_fare_tab(self):
        ...
        for tp in fetch_data("SELECT type_id, type_name, base_price, validity_minutes FROM TicketType"):
            self.tt_tree.insert('', tk.END,
                                values=(tp['type_id'], tp['type_name'], tp['base_price'], tp['validity_minutes']))
    ...
    def create_staff_tab(self):
        ...
        for c, h in zip(cols_s, ['ID', '姓名', '岗位', '联系方式']):
            ...
            self.staff_tree.column(c, width=100)
        ...
        for c, h in zip(cols_a, ['排班ID', '员工ID', '站点ID', '开始', '结束']):
            ...
            self.assign_tree.column(c, width=100)
        ...
        for a in fetch_data("SELECT assign_id,staff_id,station_id,start_time,end_time FROM StaffAssignment"):
            ...
            a['assign_id'], a['staff_id'], a['station_id'], a['start_time'], a['end_time']))
    ...
    def create_maint_tab(self):
        ...
        for c, h in zip(cols_m, headers):
            self.maint_tree.heading(c, text=h);
            self.maint_tree.column(c, width=100)
        self.maint_tree.pack(fill="both", expand=True)
        for r in fetch_data(
                "SELECT record_id,equipment,station_id,staff_id,start_time,end_time,description FROM MaintenanceRecord"):
    ...
    def create_alert_tab(self):
        ...
        for c, h in zip(cols_al, headers):
            self.alert_tree.heading(c, text=h);
            self.alert_tree.column(c, width=120)
    ...
    def create_turnstile_tab(self):
        ...
        for c, h in zip(cols_t, headers):
            ...
            self.turnstile_tree.column(c, width=100)
